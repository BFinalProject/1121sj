    ...
    <div id="wrapper">
      ...
      <div id="main">
        <div class="inner">
          ...
          <h1>{{ detailList.subject }}</h1>
          <!-- <h1>아니 시발</h1> -->
          <h3>작성자:{{ detailList.id }}</h3>
          <hr>
          <p>
            {{ detailList.content }}
            <!-- {{ detailList }} -->
          ...
          <section style="width:1300px">
              ...
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr v-for="(item, idx) in comment" :key="idx">
                    <th scope="row" v-show="comment.length != 0">{{ idx + 1 }}</th>
            ...
            <div class="input-group mb-3">
                <input type="text" class="form-control" placeholder="Recipient's username" aria-label="Recipient's username" aria-describedby="button-addon2" v-model="comments" />
              <button class="btn btn-outline-secondary" type="" id="button-addon2" @click="onSubmitForm()">답글쓰기</button>
            </div>
          </section>
        </div>
      </div>
